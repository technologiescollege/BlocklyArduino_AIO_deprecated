...
; -------------------- FK Addition -----------------
/*
Func: ini_getValuewDefault
    same as ini_getValue but defautl value is returned if empty

*/
ini_getValuewDefault(ByRef _Content, _Section, _Key, _Default, _PreserveSpace = False)
{
	v := ini_getValue(_Content, _Section, _Key, _PreserveSpace)
	If !v
		v := _Default
	Return v
}
...
Parameters:
	...
	encoding		- *Optional* Provides ability to override default ANSI 
						encoding. Possibles values are : UTF-8, UTF-16, CPnnn
						where nnn is the numeric identifier for desired codepage
...
*/
ini_load(ByRef _Content, _Path = "", _encoding = "", _convertNewLine = false)
{
    ini_buildPath(_Path)
	If (_encoding = "UTF-8")
		_froptions := "*P65001"
	Else If (_encoding = "UTF-16")
		_froptions := "*P1200"
	Else If InStr(_encoding, "CP")
		StringReplace, _froptions, _encoding, CP, *P
	Else
		_froptions := ""
    ...
    If !error
    {
        FileRead, _Content, %_froptions% %_Path%
        If ErrorLevel
            error := true
    }
    If error
...
Examples:
    ...
    *Output:*
    > E:\Tuncay\AutoHotkey\scriptname.ini
*/
ini_save(ByRef _Content, _Path = "", _encoding = "", _convertNewLine = true, _overwrite = true)
{
    ini_buildPath(_Path)
    error := false
    If (_overwrite)
    ...
    If (error = false)
    {
        If (_convertNewLine)
        {
            FileAppend, %_Content%, %_Path%, %_encoding%
        }
        Else
        {
            FileAppend, %_Content%, *%_Path%, %_encoding%
